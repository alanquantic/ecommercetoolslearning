...
export function buildLogisticsStudentEmail(payload) {
  const brief = payload.brief || {};
  const result = payload.result || {};
  const businessName = brief.businessName || "tu negocio";
  const subject = `Tus mensajes de logistica clara: ${businessName}`;

  return {
    subject,
    html: wrapEmail({
      eyebrow: "Logistica clara",
      title: "No prometas rapido; promete claro",
      subtitle: businessName,
      recipientName: brief.studentName || brief.studentEmail,
      intro:
        "Aqui tienes mensajes concretos para explicar tiempos, confirmaciones, retrasos y problemas sin sonar improvisado.",
      badges: buildLogisticsBadges(payload),
      sections: [
        {
          title: "Contraste clave",
          body: `Mensaje debil: "${result.ejemplo_debil}" Mejor version: ${result.ejemplo_mejor}`,
        },
        {
          title: "Mensajes listos",
          list: buildLogisticsMessageList(result),
        },
        {
          title: "Tip clave",
          body: result.tip_clave,
        },
      ],
      footerNote: `Puedes volver a usar la herramienta en ${payload.appBaseUrl}.`,
    }),
    text: buildTextEmail({
      subject,
      intro: "Mensajes de logistica clara listos para usar.",
      sections: [
        { title: "Contexto", list: buildLogisticsContextList(payload) },
        { title: "Contraste", body: `Debil: ${result.ejemplo_debil}\nClaro: ${result.ejemplo_mejor}` },
        { title: "Mensajes", list: buildLogisticsMessageList(result) },
        { title: "Tip clave", body: result.tip_clave },
      ],
      footerNote: `Resultado generado desde ${payload.appBaseUrl}`,
    }),
  };
...
export function buildLogisticsTeacherEmail(payload) {
  const brief = payload.brief || {};
  const result = payload.result || {};
  const studentReference = brief.studentName || brief.studentEmail;
  const businessName = brief.businessName || "Sin nombre";
  const subject = `Nuevo ejercicio de logistica: ${studentReference} | ${businessName}`;
  ...
  return {
    ...
    html: wrapEmail({
      ...
      subtitle: businessName,
      recipientName: "Profesor",
      intro: `Se genero un paquete de mensajes logisticos para ${studentReference}.`,
      badges: [
        `Alumno: ${studentReference}`,
        `Correo: ${brief.studentEmail}`,
      ...
      sections: [
        {
          title: "Contexto capturado",
          list: buildLogisticsContextList(payload),
        },
        {
          title: "Errores detectados",
          list: payload.selectedErrorLabels || [],
        },
        {
          title: "Mensajes generados",
          list: buildLogisticsMessageList(result),
    ...
    text: buildTextEmail({
      ...
      sections: [
        { title: "Alumno", body: `${studentReference} <${brief.studentEmail}>` },
        { title: "Contexto", list: buildLogisticsContextList(payload) },
        { title: "Errores detectados", list: payload.selectedErrorLabels || [] },
        { title: "Mensajes", list: buildLogisticsMessageList(result) },
      ],
      footerNote: `Sitio: ${payload.appBaseUrl}`,
    }),
...
function buildLogisticsBadges(payload) {
  return [
    payload.businessTypeLabel ? `Tipo: ${payload.businessTypeLabel}` : "",
    payload.coverageLabel ? `Cobertura: ${payload.coverageLabel}` : "",
    payload.brief?.prepTime ? `Preparacion: ${payload.brief.prepTime}` : "",
  ];
}

function buildLogisticsContextList(payload) {
  const brief = payload.brief || {};
  return [
    brief.productDescription ? `Que vende: ${brief.productDescription}` : "",
    payload.businessTypeLabel ? `Tipo: ${payload.businessTypeLabel}` : "",
    payload.coverageLabel ? `Cobertura: ${payload.coverageLabel}` : "",
    brief.prepTime ? `Preparacion: ${brief.prepTime}` : "",
    brief.shippingCost ? `Costo de envio: ${brief.shippingCost}` : "",
    brief.carriers ? `Paqueterias o metodo: ${brief.carriers}` : "",
    brief.damagePolicy ? `Proceso de danos/problemas: ${brief.damagePolicy}` : "",
  ].filter(Boolean);
}

function buildLogisticsMessageList(result) {
  return [
    result?.mensaje_entrega ? `Respuesta sobre entrega: ${result.mensaje_entrega}` : "",
    result?.confirmacion_pedido ? `Confirmacion de pedido: ${result.confirmacion_pedido}` : "",
    result?.aviso_envio ? `Aviso de envio o entrega: ${result.aviso_envio}` : "",
    result?.comunicacion_retraso ? `Comunicacion de retraso: ${result.comunicacion_retraso}` : "",
    result?.protocolo_danado ? `Protocolo de dano o problema: ${result.protocolo_danado}` : "",
  ].filter(Boolean);
}

export function buildLogiCoachStudentEmail(payload) {
  const subject = `Tu plan logistico personal: ${payload.evaluation?.level?.label || "diagnostico"}`;
  const intro = `Hola ${payload.studentName || "alumno"}, este es tu plan operativo personalizado basado en lo que respondiste en LogiCoach.`;

  const planWeeks = (payload.plan?.thirtyDayPlan || [])
    .map((week) => `${week.week} · ${week.focus}: ${week.actions.join(" / ")}`)
    .filter(Boolean);

  return {
    subject,
    html: wrapEmail({
      eyebrow: "LogiCoach · diagnostico personalizado",
      title: payload.plan?.headline || "Tu plan logistico personal",
      subtitle: payload.evaluation?.level?.detail || "",
      recipientName: payload.studentName || "alumno",
      intro,
      badges: [
        `Score: ${payload.evaluation?.score || 0}/100`,
        `Nivel: ${payload.evaluation?.level?.label || "(sin nivel)"}`,
        payload.businessActivity ? `Negocio: ${payload.businessActivity}` : "",
      ],
      sections: [
        { title: "Resumen", body: payload.plan?.summary || "" },
        { title: "Fortalezas detectadas", list: payload.plan?.strengths || [] },
        { title: "Riesgos a atender", list: payload.plan?.risks || [] },
        { title: "Plan de 30 dias", list: planWeeks },
        { title: "Checklist de esta semana", list: payload.plan?.checklist || [] },
        { title: "Indicadores a medir", list: payload.plan?.metrics || [] },
        {
          title: "Alertas activas",
          list: (payload.evaluation?.alerts || []).map((alert) => `${alert.title}: ${alert.detail}`),
        },
      ],
      footerNote: `Plan generado desde ${payload.appBaseUrl || "Ruta E-commerce"}.`,
    }),
    text: buildTextEmail({
      subject,
      intro,
      sections: [
        { title: "Resumen", body: payload.plan?.summary || "" },
        { title: "Fortalezas", list: payload.plan?.strengths || [] },
        { title: "Riesgos", list: payload.plan?.risks || [] },
        { title: "Plan 30 dias", list: planWeeks },
        { title: "Checklist", list: payload.plan?.checklist || [] },
        { title: "KPIs", list: payload.plan?.metrics || [] },
      ],
      footerNote: `Generado desde ${payload.appBaseUrl || "Ruta E-commerce"}.`,
    }),
  };
}

export function buildLogiCoachTeacherEmail(payload) {
  const studentReference = payload.studentName || payload.studentEmail;
  const subject = `LogiCoach: nuevo plan de ${studentReference} (${payload.evaluation?.score || 0}/100)`;
  const intro = `Se genero un plan personalizado para ${studentReference}.`;

  return {
    subject,
    html: wrapEmail({
      eyebrow: "LogiCoach · plan enviado al alumno",
      title: payload.plan?.headline || "Nuevo diagnostico LogiCoach",
      subtitle: payload.evaluation?.level?.label || "",
      recipientName: "Profesor",
      intro,
      badges: [
        `Alumno: ${studentReference}`,
        `Correo: ${payload.studentEmail}`,
        `Negocio: ${payload.businessActivity || "(sin descripcion)"}`,
        `Score: ${payload.evaluation?.score || 0}/100`,
      ],
      sections: [
        { title: "Resumen del plan", body: payload.plan?.summary || "" },
        {
          title: "Alertas detectadas",
          list: (payload.evaluation?.alerts || []).map((alert) => `${alert.title}: ${alert.detail}`),
        },
        { title: "Plan de 30 dias (resumen)", list: (payload.plan?.thirtyDayPlan || []).map((week) => `${week.week}: ${week.focus}`) },
      ],
      footerNote: `Acceso: ${payload.appBaseUrl || ""}`,
    }),
    text: buildTextEmail({
      subject,
      intro,
...
export function buildBingoAnecdoteEmail(payload) {
  const errorLabel = payload.errorLabel ? `${payload.errorLabel}` : "(sin etiqueta)";
  const subject = `LogiBingo · carta cantada: ${errorLabel}`;
  const intro = `Un alumno acaba de compartir una anecdota anonima vivida como cliente cuando le toco esta carta en LogiBingo.`;
  const anecdoteSafe = payload.anecdote || "(sin anecdota escrita)";

  return {
    subject,
    html: wrapEmail({
      eyebrow: "LogiBingo · loteria del e-commerce",
      title: errorLabel,
      subtitle: payload.linesCompleted
        ? `Linea ${payload.linesCompleted} cantada · ${payload.errorsMarked || 0} de 16 cartas marcadas`
        : `Anecdota anonima nueva`,
      recipientName: "Profesor",
      intro,
      badges: [
        payload.tableNumber ? `Mesa ${payload.tableNumber}` : "",
        payload.timestamp ? `Recibido: ${payload.timestamp}` : "",
      ],
      sections: [
        {
          title: "Lo que vivio como cliente",
          body: anecdoteSafe,
        },
        {
          title: "Contexto del tablero",
          list: [
            payload.errorsMarked != null ? `Cartas marcadas al enviar: ${payload.errorsMarked} de 16` : "",
            payload.linesCompleted != null ? `Lineas cantadas: ${payload.linesCompleted}` : "",
            payload.dangerLevel ? `Indice del alumno: ${payload.dangerLevel}` : "",
          ].filter(Boolean),
        },
      ],
      footerNote: `Capturado desde ${payload.appBaseUrl || "LogiBingo"}.`,
    }),
    text: buildTextEmail({
      subject,
      intro,
      sections: [
        { title: "Anecdota como cliente", body: anecdoteSafe },
        { title: "Carta que se canto", body: errorLabel },
        {
          title: "Contexto del tablero",
          list: [
            payload.errorsMarked != null ? `Cartas marcadas: ${payload.errorsMarked} de 16` : "",
            payload.linesCompleted != null ? `Lineas cantadas: ${payload.linesCompleted}` : "",
            payload.dangerLevel ? `Indice del alumno: ${payload.dangerLevel}` : "",
          ].filter(Boolean),
        },
      ],
      footerNote: `Recibido desde ${payload.appBaseUrl || "LogiBingo"}.`,
    }),
  };
}
