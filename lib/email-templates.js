...
export function buildBingoAnecdoteEmail(payload) {
  ...
  const subject = `LogiBingo · anecdota anonima: ${errorLabel}`;
  const intro = `Un alumno acaba de compartir una anecdota anonima al completar una linea en LogiBingo.`;
  const anecdoteSafe = payload.anecdote || "(sin anecdota escrita)";

  return {
    subject,
    html: wrapEmail({
      eyebrow: "LogiBingo · anecdota anonima",
      title: errorLabel,
      subtitle: payload.linesCompleted
        ? `Linea ${payload.linesCompleted} completada · ${payload.errorsMarked || 0} de 16 errores marcados`
      ...
      sections: [
        {
          title: "Anecdota compartida",
          body: anecdoteSafe,
        },
        {
          title: "Contexto",
          list: [
            payload.errorsMarked != null ? `Errores marcados al enviar: ${payload.errorsMarked} de 16` : "",
            payload.linesCompleted != null ? `Lineas completadas: ${payload.linesCompleted}` : "",
            payload.dangerLevel ? `Nivel de peligro: ${payload.dangerLevel}` : "",
          ].filter(Boolean),
        },
      ],
    ...
    text: buildTextEmail({
      ...
      intro,
      sections: [
        { title: "Anecdota", body: anecdoteSafe },
        { title: "Error marcado", body: errorLabel },
        {
          title: "Contexto",
          list: [
            payload.errorsMarked != null ? `Errores marcados: ${payload.errorsMarked} de 16` : "",
            payload.linesCompleted != null ? `Lineas completadas: ${payload.linesCompleted}` : "",
            payload.dangerLevel ? `Nivel: ${payload.dangerLevel}` : "",
          ].filter(Boolean),
        },
      ],
