...
export async function requestAiAnalysis(context) {
  // En la siguiente fase, este mock se reemplaza por fetch('/api/analyze', ...)
  // para llamar al modelo desde backend sin exponer la API key.
  await new Promise((resolve) => window.setTimeout(resolve, 900));
  return buildMockAiAnalysis(context);
}

function buildMockAiAnalysis(context) {
